...
출력:
{"1": 65, "2": 45, "3": 65, "4": 85, "5": 85, "6": 85, "7": 65, "8": 45, "9": 65, "10": 85}

근거:
- KPI 1(65): 구현 언급은 거의 없음, 언어·프레임워크 실전 증거 부족 → 보통 중 (인프라형 BE라서 정상)
- KPI 2(45): API, 리소스, 응답, 에러, 설계 언급 없음 → 하
- KPI 3(65): 쿼리 최적화 언급은 있으나 ERD·스키마·인덱스 구조는 없음 → 보통 중
- KPI 4(85): 클라우드 → 하이브리드 구조 전환, 구조 재정의 + 판단 주도, 확장성·비용·안정성 트레이드오프 → 강한 상
- KPI 5(85): 클라우드 배포, 물리 서버, Docker, 운영 환경 구성 → 강한 상
- KPI 6(85): 트래픽 증가 문제, 구조 전환, 컴퓨팅 자원 8배 확장 → 강한 상
- KPI 7(65): 방화벽 설정, 보안 체계 구축, 인증/권한은 아니지만 인프라 보안 → 보통 중
- KPI 8(45): 테스트, 품질, 리팩토링 전혀 없음 → 하
- KPI 9(65): "제안하고 실행을 주도"는 있으나 협업 구조·문서 증거는 없음 → 보통 중
- KPI 10(85): 운영 비용, 모니터링 환경, 안정성 확보 → 강한 상
...
출력:
{"1": 45, "2": 45, "3": 65, "4": 85, "5": 85, "6": 85, "7": 65, "8": 45, "9": 65, "10": 85}

근거:
- KPI 1(45): 코드, 언어, 프레임워크 전혀 언급 없음, 전부 인프라/구조 이야기 → 하
- KPI 2(45): API, 리소스, 에러, 응답 전혀 없음 → 하
- KPI 3(65): 쿼리 최적화 언급은 있으나 DB 설계·모델링·인덱스 구조는 없음 → 보통 중
- KPI 4(85): 앱 레벨 → 인프라 레벨 전환, 하이브리드 구조, 구조적 재정의 + 실행 주도 → 강한 상
- KPI 5(85): 클라우드, 물리 서버, Docker, 배포 환경 구성 → 강한 상
- KPI 6(85): 트래픽 증가, 구조 전환, 비용·성능 동시 개선 → 강한 상
- KPI 7(65): 방화벽, 보안 정책, 인증/권한은 아니지만 인프라 보안 → 보통 중
- KPI 8(45): 테스트, 리팩토링, 품질 전혀 없음 → 하
- KPI 9(65): "제안하고 실행을 주도"는 있으나 협업 구조, 문서 증거는 없음 → 보통 중
...
출력:
{"1": 65, "2": 65, "3": 65, "4": 45, "5": 65, "6": 65, "7": 45, "8": 45, "9": 65, "10": 65}

근거:
- KPI 1(65): REST API 구현, ORM 사용, 실서비스 개발, 깊은 언어 활용·예외 처리·고급 패턴 증거는 없음 → 보통 중
- KPI 2(65): REST API 구현, 요구사항 기반 엔드포인트 작성, 리소스 설계·에러 규약·버전 전략 없음 → 보통 중
- KPI 3(65): 테이블 설계, 인덱스 추가로 성능 개선, 구조적 모델링·대용량 고려는 없음 → 보통 중
- KPI 4(45): "팀에서 정한 방향에 따라 수행", 구조 설계·의사결정 없음 → 하
- KPI 5(65): 클라우드 배포, 기본 인프라 설정, CI/CD, 자동화, 구조 설계는 없음 → 보통 중
- KPI 6(65): API 성능 저하 인지, 인덱스 추가, 캐시, 큐, 병목 분석 수준은 아님 → 보통 중
- KPI 7(45): 인증·권한·보안 설계 언급 없음 → 하
- KPI 8(45): 테스트, 리팩토링, 품질 관리 언급 없음 → 하
- KPI 9(65): "팀 내 논의", 공동으로 인덱스 결정, 문서·정책·리딩은 아님 → 보통 중
- KPI 10(65): 로그 확인, 장애 원인 파악, 수정·복구, 모니터링 체계·운영 루프는 없음 → 보통 중
